...
-- Module Name: fft_buffer - rtl
-- Project Name: Chameleon
--
-- Revision 0.04 - Apply Hann window on WRITE instead of READ.
--
-- Previous design applied the window during a 3-stage read pipeline,
-- meaning the buffer had to fill AND then be processed before streaming.
-- This caused m_tlast to fire at ~6Hz instead of 93.75Hz.
--
-- New design:
--   WRITE: sample_in * hann_coeff -> stored as 32-bit windowed value
--          This uses a 2-cycle multiply pipeline:
--          Cycle 1: latch sample_in and hann_coeff for write_ptr
--          Cycle 2: multiply, store result in buffer
--          Buffer stores 32-bit windowed samples ready to stream.
--
--   READ:  Simply stream buf[read_ptr] directly to m_tdata.
--          No windowing pipeline needed - data already windowed.
--          One sample per cycle when m_tready='1'.
--
-- Buffer stores windowed value scaled back to 24-bit range:
--   sample_in: Q23 (24-bit signed)
--   hann_coeff: 0..32767 (Q15, 16-bit)
--   product: Q38 (40-bit signed)
--   stored: product >> 15 = Q23, sign-extended to 32 bits
--
-- Ping-pong: buf_a fills while buf_b streams, then swap.
----------------------------------------------------------------------------------
...
architecture rtl of fft_buffer is

    -- Ping-pong buffers store 32-bit windowed samples - force BRAM inference
    type ram_t is array (0 to 1023) of std_logic_vector(31 downto 0);
    signal buf_a      : ram_t := (others => (others => '0'));
    signal buf_b      : ram_t := (others => (others => '0'));
    attribute ram_style : string;
    attribute ram_style of buf_a : signal is "block";
    attribute ram_style of buf_b : signal is "block";

    signal write_buf  : std_logic := '0';  -- 0=write A, 1=write B
    signal read_buf   : std_logic := '0';  -- latched when buf_ready fires
    signal write_ptr  : integer range 0 to 1023 := 0;
    signal read_ptr   : integer range 0 to 1023 := 0;
    signal buf_ready  : std_logic := '0';
    signal sending    : std_logic := '0';

    -- Write pipeline (2 cycles: latch -> multiply -> store)
    signal pipe_valid : std_logic := '0';
    signal pipe_ptr   : integer range 0 to 1023 := 0;
    signal pipe_buf   : std_logic := '0';
    signal pipe_samp  : signed(23 downto 0) := (others => '0');
    signal pipe_coeff : signed(15 downto 0) := (others => '0');
    signal pipe_prod  : signed(39 downto 0) := (others => '0');
    signal pipe_last  : std_logic := '0';  -- signals frame complete

    -- Hann window ROM (0..32767 scaled Q15)
    type hann_rom_t is array (0 to 1023) of integer range 0 to 32767;
    constant HANN_ROM : hann_rom_t := (
        0, 0, 1, 3, 5, 8, 11, 15,
...
begin
    ...
    -- ----------------------------------------------------------------
    -- Write process with 2-cycle Hann window pipeline:
    --   Cycle 1 (on sample_valid): latch sample + coefficient
    --   Cycle 2: multiply, write windowed result to buffer
    -- ----------------------------------------------------------------
    process(sys_clk)
    begin
        if rising_edge(sys_clk) then
            if rst = '1' then
                ...
                -- Clear buf_ready once read side picks it up
                if sending = '1' then
                    buf_ready <= '0';
                end if;

                -- Pipeline stage 1: latch sample and coefficient
                pipe_valid <= '0';
                pipe_last  <= '0';
                if sample_valid = '1' then
                    pipe_samp  <= x"010000"; --signed(sample_in);
                    pipe_coeff <= to_signed(HANN_ROM(write_ptr), 16);
                    pipe_ptr   <= write_ptr;
                    pipe_buf   <= write_buf;
                    pipe_valid <= '1';
                    ...
                    if write_ptr = 1023 then
                        write_ptr <= 0;
                        write_buf <= not write_buf;
                        read_buf  <= write_buf;  -- latch which buf just completed
                        buf_ready <= '1';
                        pipe_last <= '1';
                    else
                ...
                -- Pipeline stage 2: multiply and store
                if pipe_valid = '1' then
                    pipe_prod <= pipe_samp * pipe_coeff;
                    -- Store in correct buffer: shift right 15 (Q38->Q23),
                    -- sign-extend to 32 bits
                    if pipe_buf = '0' then
                        buf_a(pipe_ptr) <= std_logic_vector(
                            resize(pipe_prod(38 downto 15), 32));
                    else
                        buf_b(pipe_ptr) <= std_logic_vector(
                            resize(pipe_prod(38 downto 15), 32));
                    end if;
                end if;
            end if;
    ...
    -- ----------------------------------------------------------------
    -- Read process: stream windowed samples directly
    -- Present current read_ptr data, then increment
    -- ----------------------------------------------------------------
    process(sys_clk)
    begin
        if rising_edge(sys_clk) then
            if rst = '1' then
                ...
                -- Start sending when buffer ready
                if buf_ready = '1' and sending = '0' then
                    sending  <= '1';
                    read_ptr <= 0;
                end if;

                -- Stream one sample per cycle
                if sending = '1' and m_tready = '1' then
                    -- Present current sample
                    if read_buf = '0' then
                        m_tdata <= buf_a(read_ptr);
                    else
                        m_tdata <= buf_b(read_ptr);
                    end if;
                    m_tvalid <= '1';
